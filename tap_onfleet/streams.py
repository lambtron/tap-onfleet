...
class Stream():
    ...
    def update_bookmark(self, state, value):
        if self.is_bookmark_old(state, value):
            singer.write_bookmark(state, self.name, self.replication_key, time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(value / 1000)))


    def is_bookmark_old(self, state, value):
        current_value = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(value / 1000))
